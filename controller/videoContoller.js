...
// Get all videos with optional search and category filtering
export const getVideos = async (req, res) => {
  try {
    // Extract query parameters from request URL
    // Example: /api/videos?search=react&category=Education
    ...
    // Perform case-insensitive partial match on video title
    if (search) {
      filter.title = { $regex: search, $options: "i" };
    }

    // If category filter exists and is not "All"
    // Add category condition to filter
    if (category && category !== "All") {
      filter.category = category;
    }

    // Fetch videos from database based on filter
    const videos = await Video.find(filter)

      // Populate channel information (only channelName field)
      .populate("channel", "channelName")

      // Populate uploader username
      .populate("uploader", "username")

      // Populate uploader avatar as well
      .populate("uploader", "username avatar")

      // Sort videos by newest first
      .sort({ createdAt: -1 });

    // Send response with total videos count and video list
    res.json({
      total: videos.length,
      videos,
    });
...
export const toggleLike = async (req, res) => {
  try {
    ...
    res.json({
      ...
      likes_list: video.likes,
      dislikes_list: video.dislikes,
    });
  } catch (error) {
